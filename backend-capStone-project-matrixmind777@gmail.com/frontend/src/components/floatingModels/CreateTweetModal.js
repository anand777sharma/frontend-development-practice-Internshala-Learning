...
const CreateTweetModal = () => {
    ...
    const submitHandler = async (e) => {
        ...
        const url = "http://localhost:5000/api/file/files/" + imgRes.data.fileName
        // console.log(url);//"http://localhost:5000/api/file/files/file-1705648616517-0.8677161598305487.png"
        console.log(pic);
        setPictureState("hyyy")
        console.log(pic+":after");
        // setPic("hyyy");
        // setTest("data from test")
        // setTimeout(() => {
        //     setTweet({ ...tweet, picture: picture })
        // }, 2000);
        // console.log("test is test" + test)
        // setTweet({ ...tweet, picture: url })
        //  setTweet(prevTweet => ({ ...prevTweet, picture: url })); //also used this but not woked

        // }

        const resp = await axios.post('http://localhost:5000/api/tweet/', tweet, {
